...
class SimpleVO:
	...
	def run(self):
		...
		o3d.io.write_line_set('vo_kitti.ply', all_cameras)
		vis.destroy_window()
		p.join()
	...
	def process_frames(self, queue):
		...
		queue.put((R, t))
		base_scale = 1
		img0 = cv.imread(self.frame_paths[0])
		img1 = cv.imread(self.frame_paths[1])
		kp0, des0 = find_features(img0)
		kp1, des1 = find_features(img1)
		matches01 = match_features(des0, des1)
		R01, t01 = compute_pose(kp0, kp1, matches01, self.K)
		R = R @ R01
		t = t + np.dot(R, t01)
		queue.put((R, t))
		...
		for frame_path in self.frame_paths[2:]:
			...
			R12, t12 = compute_pose(kp1, kp2, matches12, self.K)
			cloud01 = triangulate(kp0, kp1, R01, t01, self.K, [m for m in matches01 if m.trainIdx in [m12.queryIdx for m12 in matches12]])
			cloud12 = triangulate(kp1, kp2, R12, t12, self.K, [m for m in matches12 if m.queryIdx in [m01.trainIdx for m01 in matches01]])
			...
			R01 = R12.copy()
			t01 = t12.copy()
			for pts in [kp2[m.trainIdx].pt for m in matches12]:
